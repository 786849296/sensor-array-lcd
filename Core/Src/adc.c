...
void MX_ADC1_Init(void)
{
  ...
  */
  ...
  LL_DMA_ConfigAddresses(DMA1, LL_DMA_CHANNEL_1, LL_ADC_DMA_GetRegAddr(ADC1, LL_ADC_DMA_REG_REGULAR_DATA), (uint32_t)&adcRaw, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
  LL_DMA_SetDataLength(DMA1, LL_DMA_CHANNEL_1, 4u);
  ...
  */
  ...
  /*       calibration.                                                     */
  __IO uint32_t backUp_adcDmaTransfer = LL_ADC_REG_GetDMATransfer(ADC1);
  LL_ADC_REG_SetDMATransfer(ADC1, LL_ADC_REG_DMA_TRANSFER_NONE);
  LL_ADC_Enable(ADC1);
  for (int i = 0; i < LL_ADC_DELAY_ENABLE_CALIB_ADC_CYCLES * 16; i++);
  LL_ADC_StartCalibration(ADC1);
  while (LL_ADC_IsCalibrationOnGoing(ADC1));
  LL_ADC_REG_SetDMATransfer(ADC1, backUp_adcDmaTransfer);
